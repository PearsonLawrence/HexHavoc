...
//Utilized basic tutorial from code monkey to understand fundamentals and then added on and created entire UI system from scratch. This script is a heavily modified version to account for custom auto load logic.
...
public class GameLobbyComponent : MonoBehaviour
{
    ...
    public async void JoinLobbyByLobby(Lobby lobby, UnNetworkPlayer P)
    {
        ...
        PrintPlayers(joinedLobby);
        isJoined = true;
        XR_Player.GetComponent<UnNetworkPlayer>().isJoining = true;
        ...
        if (joinedLobby != null) //If you are connected to a lobby
        {
            ...
            if (lobbyUpdateTimer < 0f)
            {
                ...
                if (joinedLobby.Data["StartGame"].Value != "0") //if game has not started
                {
                    ...
                    {
                        isLobbyStart = await currentRelay.JoinRelay(joinedLobby.Data["StartGame"].Value, XR_Player.GetComponent<UnNetworkPlayer>()); //update lobby start
                    }
                    joinedLobby = null;
